...
function getCanvasContext(canvas: HTMLCanvasElement) {
  ...
  const draw = (reserve = false, callback) => {
    try {
      ...
      // canvas 未挂载或者大小为0可能导致渲染失败, 所以这里加上延时
      requestAnimationFrame(() => {
        copy.forEach(({ func, args }) => {
          func.apply(ctx, args);
        });
      });
      emptyActions();
...
function createH5CavansRef() {
  let _current = null;
  const ref = function (_v) {
    if (_v) {
      const canvas = _v.canvasRef;
      _current = getCanvasContext(canvas);
    }
  };
  // 同时可以通过函数的形式调用
